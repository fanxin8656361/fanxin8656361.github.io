<template>
    ...
    <div class="main-box">
      ...
      <div class="main">
        ...
        <div class="line1">
          ...
          <p class="desc">
            图标为保证统一性，我们使用统一了图标的样式： 宽度：1px 圆角：0 - 2
            图标尺寸为24px 并可以制作成为图标字体供技术使用。
            采用细线尖端icon，与官网保持一致。
          </p>
        </div>
        <div class="nav-tabs" :id="catalogue[1]">
          ...
          <a
            class="nav-more"
            :href="`${staticUrl}/zip/download.zip`"
            target="_blank"
            download="iconfont.zip"
            rel="noopener norefferrer"
          >
            下载字体图标
            <!-- <i class="Audi-Dash Audi-Dashicon-download"></i> -->
            <i class="Audi-Dash Audi-Dashdown-small"></i>
          </a>
        ...
        <div class="tab-container">
          ...
          <div class="content font-class" style="display: block">
            <ul class="icon_lists dib-box">
              ...
              <li class="dib">
                <span
                  class="icon Audi-Dash Audi-Dashcategory-management"
                ></span>
                <div class="name">category-management</div>
        ...
        <div class="article markdown" v-if="tableIndex == 2" :id="catalogue[2]">
          <div class="lable" id="symbol-">Symbol 引用</div>
              <p>
 ...
 &lt;/svg&gt;</code></pre>
        ...
        <div class="article markdown" v-if="tableIndex == 0" :id="catalogue[2]">
          <div class="lable" id="unicode-">Unicode 引用</div>
...
.Audi-Dash {
  ...
  &lt;span class="Audi-Dash"&gt;&amp;#x33;&lt;/span&gt;
        ...
        <div class="article markdown" v-if="tableIndex == 1" :id="catalogue[2]">
              <div class="lable" id="font-class-">font-class 引用</div>
              <div >
                font-class 是 Unicode 使用方式的一种变种，主要是解决 Unicode
                书写不直观，语意不明确的问题。
              </div>       
              <div>与 Unicode 使用方式相比，具有如下特点：</div>
              <ul>
                <li>
                ...
                <li>
                  因为使用 class 来定义图标，所以当要替换图标时，只需要修改
                  class 里面的 Unicode 引用。
                </li>
              </ul>
              <div class="introduce">使用步骤如下：</div>
              <div id="-fontclass-">
  ...
  &lt;link rel="stylesheet" href="./iconfont.css"&gt;
              ...
              <div id="-">第二步：挑选相应图标并获取类名，应用于页面：</div>
              <pre><code class="language-html">
  &lt;span class="Audi-Dash Audi-Dashxxx"&gt;&lt;/span&gt;
  </code></pre>
              <blockquote>
                <p>
                  " Audi-Dash" 是你项目下的
    ...
    <div class="catalogue">
      <span class="cata-item title">本页重点</span>
      <!-- 锚点无法使用#已被哈希占用
                                    <a class="cata-item" v-for="(item, index) in catalogue" :key="index" :href="'#' + item">{{item}}</a> -->
      <span
        class="cata-item pointer"
...
export default {
  ...
  data() {
    return {
      ...
      content1:
        "在设计规范中，图标也是一个非常重要的元素，它可以帮助用户更快速、直观地理解信息和完成操作。图标可以用来代表特定的功能、操作或内容，以及传达特定的情感和氛围，因此在设计中选择适合的图标非常关键。比如，在移动应用程序中，常常使用各种图标来代表不同的功能和操作，如放大镜图标代表搜索功能，心形图标代表喜欢功能等等。",
    };
  },
  mounted() {
    const that = this;
    ...
    $(".icon_lists .dib").click(function (e) {
      ...
      switch (that.tableIndex) {
        case 0:
          ...
          code = `<i class="icon Audi-Dash">${content}</i>`;
          break;

        case 1:
          content = $(this).children(".name").text().trim() || "";
          // code = `<i class="icon Audi-Dash icon-${content}"></i>`;
          code = `<i class="Audi-Dash Audi-Dash${content}"></i>`;
          break;

        case 2:
          content = $(this).children(".code-name").text().trim() || "";
          code = `<svg class="icon svg-icon" aria-hidden="true">\n\t<use xlink:href="${content}"></use>\n</svg>`;
        default:
          break;
      }
